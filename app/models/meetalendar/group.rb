class Meetalendar::Group < ApplicationRecord
  ...
  def self.all_upcoming_events(time_now = nil)
    ...
    upcoming_events = Meetalendar::MeetupApi.find_upcoming_events(Meetalendar::Frame.meetup_query_events).select do |event|
      event.start_time > time_now and event.start_time < time_now + 3.months and group_ids.include?(event.group_id)
    end
    ...
    series_events = upcoming_events.select(&:series?).map do |series|
      Meetalendar::MeetupApi.group_urlname_events(series.group_urlname, page: 5).select do |event|
        event.start_time > time_now and event.start_time < time_now + 3.months
      end
    end.flatten
