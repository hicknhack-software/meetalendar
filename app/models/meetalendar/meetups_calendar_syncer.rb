...
module Meetalendar
  module MeetupsCalendarSyncer
    ...
    def self.get_path_authorized(path, args = {})
      ...
      current_tokens = AuthCredential.for_auth('meetup')
      if current_tokens.nil?
      ...
      if result.status == Rack::Utils::status_code(:unauthorized)
        ...
        current_tokens = AuthCredential.store_auth('meetup', {
          'client_id': meetup_credentials['client_id'],
        ...
        JSON.parse(result&.body.nil? ? {}.to_s : result.body.to_s)
      end
    end
