...
module Meetalendar
  module MeetupsControllerLogic
    ...
    def self.search_result(parameters, time_now)
      ...
      request_result = MeetupsCalendarSyncer.get_path_authorized("/find/groups", group_params.merge({"fields" => "last_event"}))
      groups = request_result.nil? ? {} : request_result

      groups_id_name = groups.map{|g| {id: g["id"].to_i, name: g["name"].to_s, link: g["link"].to_s} }
      group_ids = groups.map{|g| g["id"]}
      request_result = MeetupsCalendarSyncer.get_path_authorized("/find/upcoming_events", {"page": 200})
      ...
      meetup_groups = groups.map{|g| MeetupGroup.new({"group_id": g["id"], "name": g["name"], "approved_cities": "", "group_link": g["link"]})}

      search_result = SearchResult.new(groups_id_name, found_upcoming_grouped_events, found_last_grouped_events, meetup_groups)
    end
  end
end
